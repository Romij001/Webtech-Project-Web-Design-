function Candidatessearch(pForm) {
	...
	xhttp.onload = function() {
		...
		if(response != null && response != "" && response != "[]")
		{
			...
			for(i=0; i < vagents.length; i++)
			{
			    ...
			    table_rows += '<td>';
			          ...
			          if(status == 1){
			            table_rows +='<form  method="post" action="../View/CandidateEdit.php">';

			            table_rows +='<input type="hidden" name="CandidateID" value="' + id + '">';
		...
		{
			table_rows = 'No Candidates(s) found';
		}

		document.getElementById("C_tbody").innerHTML = table_rows;
	}
